...
train_df, test_df = data.get_data(df=data_df, profiles=profiles, nTrain=150, nTest=50)
train_in, train_out, test_in, test_out = data.prep_data(data=[train_df, test_df])
dat = [train_in, train_out, test_in, test_out]

ren_train_df, ren_test_df = data.get_data(df=ren_data_df, profiles=ren_profiles)
...
models = nnetwork.run_Model(data=dat, neurons=neurons, activation=activations, test_df=test_df, mod='mlp_tf',
