...
def _file_stem(file):
    return Path(getattr(file, 'path', file)).stem
...
def _remove_known_rennes_links(links):
    for ignored_link in ('centrepoussee/index.html', 'clouet/index.html'):
        if ignored_link in links:
            links.remove(ignored_link)


def _case_source(directory):
    normalised = Path(directory).as_posix().lower().rstrip('/')
    if 'rennes-dat/case-dat' in normalised:
        return 'rennes'
    if 'exp-dat/case-dat' in normalised:
        return 'exp'
    if normalised.endswith('case-dat'):
        return 'aft'
    raise ValueError(f'Cannot infer case source from directory: {directory}')
...
def get_AFT_cases(directory='dat/case-dat'):
    ...
    html_all = _urlopen("http://airfoiltools.com/search/airfoils").read()
    ...
    filenames_dir = [file.name.lower() for file in os.scandir(directory)]
    linknames_new = []
    ...
    for link in links_new:
        html_foil = _urlopen(baseFlpth + link).read()
        ...
        for link_Re in links_Re:
            ...
            html_Re = _urlopen(baseFlpth + link_Re).read()
            ...
            _download_file(baseFlpth + link_csv, fullfilename)
...
def get_RENNES_cases(directory='dat/rennes-dat/case-dat'):
    ...
    html_all = _urlopen(baseFlpth).read()
    soup_all = BeautifulSoup(html_all, 'html.parser')

    links_all = [link['href'] for link in soup_all.find_all('a', href=re.compile("/index"))]
    _remove_known_rennes_links(links_all)

    linknames = [link[:-11].lower() for link in links_all]
    filenames_dir = [_file_stem(file).split('-')[0].lower() for file in os.scandir(directory)]
    linknames_new = []
    for name in linknames:
        count = 0
        for filename in filenames_dir:
    ...
    for link in links_new:
        try:
            ...
            for link_Re in links_Re:
                ...
                fullfilename = os.path.join(directory, name)
                _download_file(baseFlpth + link[:-10] + link_Re, fullfilename)
    ...
    if failures:
        print(f' Warning: {len(failures)} Rennes case page(s) failed to process.')
        print(f' Failed links: {failures[:10]}')
...
def read_RENcase(file):
    ...
    name = _file_stem(file).split('-')[0].replace('_', '-')
    re_line = next((line for line in dat if 'Re' in line and '=' in line), '')
    re_match = re.search(r'Re\s*=\s*([+-]?\d+(?:\.\d+)?)\s*e\s*([+-]?\d+)', re_line, re.IGNORECASE)
    if not re_match:
        raise ValueError(f'Could not parse Reynolds number from line: {re_line!r}')
    Re = float(re_match.group(1)) * 10 ** int(re_match.group(2))
    dat = dat[12:]
...
def create_cases(directory='dat/case-dat', ext='csv'):
    ...
    if failures:
        print(f' Warning: {len(failures)} case file(s) failed to process.')
        print(f' Failed files: {failures[:10]}')
    return cases_df
