...
from keras.layers import Dense, BatchNormalization, Activation, InputLayer, LeakyReLU, PReLU
...
def accuracy(y_true, y_pred):
    ...
    for t, p in zip(y_true, y_pred):
        if np.abs(t - p) < 0.01:
            ...
            pass
    if len(y_pred) == 0:
        raise ValueError('Cannot calculate accuracy for an empty prediction array.')

    score = indx / len(y_pred)
    return score


def _safe_ratio(numerators, denominators):
    return [
        np.nan if np.isclose(denominator, 0.0) else numerator / denominator
        for numerator, denominator in zip(numerators, denominators)
    ]


def _prediction_pairs(df):
    if df.empty:
        raise ValueError('output_df is empty; there are no prediction rows to process.')
    return df[['file', 'Re']].drop_duplicates().sort_values(['file', 'Re'], ignore_index=True)


def _resolve_prediction_pair(df, name=None, re=None):
    available = _prediction_pairs(df)

    if name is not None and re is not None:
        exact_match = available[(available.file == name) & (available.Re == re)]
        if not exact_match.empty:
            return name, re

    if name is not None:
        name_match = available[available.file == name]
        if not name_match.empty:
            selected = name_match.iloc[0]
            print(
                f' Warning: no prediction rows found for file={name!r} and Re={re!r}. '
                f'Using available Re={selected.Re!r} for file={name!r}.'
            )
            return selected.file, selected.Re

    if re is not None:
        re_match = available[available.Re == re]
        if not re_match.empty:
            selected = re_match.iloc[0]
            print(
                f' Warning: no prediction rows found for file={name!r} and Re={re!r}. '
                f'Using available file={selected.file!r} for Re={re!r}.'
            )
            return selected.file, selected.Re

    selected = available.iloc[0]
    if name is not None or re is not None:
        print(
            f' Warning: no prediction rows found for file={name!r} and Re={re!r}. '
            f'Using file={selected.file!r}, Re={selected.Re!r}.'
        )
    return selected.file, selected.Re
...
class Model:
    ...
    def __init__(self, data, neurons, activation, weights, name, test_df, EPOCHS=50, BATCH=256, lr=0.001, verbose=0,
        ...
        self.pred, self.Pmetrics_df, self.output_df = self.predict(_model=self.model, test_in=self.test_in,
    ...
    def train(self):
        ...
        callbacks = [reduce_lr] if self.callbacks else None
    ...
    @staticmethod
    def predict(_model, test_in, test_out, test_df):
...
def run_Model(data, neurons, activation, weights, test_df, EPOCHS=50, BATCH=256, lr=0.1, verbose=0, callbacks=None):
    ...
    for act in activation:
        ...
        dup = len([path for path in model_dir.iterdir() if name in path.name])
        if dup >= 1:
            name = f'{name}-{dup}'
...
def predictions(aerofoils_df, output_df=None, name=None, re=None, plot=True, err=False):
    ...
    if output_df is None:
        raise ValueError('output_df is required to process predictions.')
    ...
    name, re = _resolve_prediction_pair(df, name=name, re=re)
    plot_df = df[df.Re == re]
    ...
    if plot:
        ...
        if show_error:
            ...
            error_values = [np.abs(tt - pp) for tt, pp in zip(t, p)]
            axs00.bar(a, error_values, width=(max(a) - min(a)) / (2 * len(a)), alpha=0.1, label='Error')
            error_max = np.nanmax(error_values)
            if not np.isnan(error_max) and error_max > 0:
                axs00.set_ylim(0, 1.5 * error_max)
        ...
        if show_error:
            ...
            error_values = [np.abs(tt - pp) for tt, pp in zip(t, p)]
            axs01.bar(a, error_values, width=(max(a) - min(a)) / (2 * len(a)), alpha=0.1, label='Error')
            error_max = np.nanmax(error_values)
            if not np.isnan(error_max) and error_max > 0:
                axs01.set_ylim(0, 1.5 * error_max)
        ...
        if show_error:
            ...
            error_values = [np.abs(tt - pp) for tt, pp in zip(t, p)]
            axs10.bar(a, error_values, width=(max(a) - min(a)) / (2 * len(a)), alpha=0.1, label='Error')
            error_max = np.nanmax(error_values)
            if not np.isnan(error_max) and error_max > 0:
                axs10.set_ylim(0, 1.5 * error_max)
            axs10.legend(bbox_to_anchor=(0.25, 1))

        profile_matches = aerofoils_df.loc[aerofoils_df.file == name]
        if profile_matches.empty:
            raise ValueError(f'No aerofoil profile found for file={name!r}.')
        aindx = profile_matches.index[0]
        aerofoils.plot_profile(aerofoils_df, aindx, scatt=False, x_val=None, ax=axs[1, 1], prnt=False)
