...
def _profile_file_name(file):
    return _file_stem(file).lower().replace('.', '').replace('_', '-')
...
def get_UIUC_foils(directory='dat/aerofoil-dat'):
    ...
    html_page = _urlopen("https://m-selig.ae.illinois.edu/ads/coord_database.html")
    ...
    filenames_dir = [_file_stem(file).lower() for file in os.scandir(directory)]
    ...
    for link in links_new:
        ...
        _download_file(baseFlpth + link, fullfilename)
...
def get_AFT_foils(directory='dat/aerofoil-dat'):
    ...
    for link in links_new:
        ...
        _download_file(baseFlpth + link_dat, fullfilename)
...
def get_RENNES_foils(directory='dat/rennes-dat/aerofoil-dat'):
    ...
    for link in links_new:
        try:
            ...
            for link_dat in links_dat:
                ...
                _download_file(baseFlpth + link[:-10] + link_dat, fullfilename)
    ...
    if failures:
        print(f' Warning: {len(failures)} Rennes aerofoil page(s) failed to process.')
...
def plot_profile(df, indx, scatt=False, x_val=None, pltfig=1, ax=None, prnt=False):
    ...
    if x_val is not None and x_val is not False:
...
class Profile:
    ...
    def coord_profile(self):
        ...
        file = _profile_file_name(self.file)
        ...
        xs_up_max = max([x for x in xs_up if x <= 5], default=max(xs_up))
        for indx, x in enumerate(xs_up):
            if x > 5:
                xs_up_max += 0.00002
                xs_up[indx] = xs_up_max

        xs_low_max = max([x for x in xs_low if x <= 5], default=max(xs_low))
        for indx, x in enumerate(xs_low):
            if x > 5:
                xs_low_max += 0.00002
                xs_low[indx] = xs_low_max
